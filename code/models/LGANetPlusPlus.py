...
class Encoder_Res(nn.Module):
    ...
    def __init__(self, in_channel=1, first_out_channel=8):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(c, 2*c, 3, 1, 1),#80
            # ResBlock(2*c),
            ResBlock(2*c))
        self.pool1 = nn.AvgPool2d(2)

        self.conv2 = nn.Sequential(
            nn.Conv3d(2*c, 4*c, 3, 1, 1),#40
            # ResBlock(4*c),
            ResBlock(4*c))
        self.pool3 = nn.AvgPool2d(2)

        self.conv3 = nn.Sequential(
            nn.Conv3d(4*c, 8*c, 3, 1, 1),#20
            # ResBlock(8*c),
            ResBlock(8*c))
        self.pool3 = nn.AvgPool2d(2)
        
        self.conv4 = nn.Sequential(
            nn.Conv3d(8*c, 16*c, 3, 1, 1),#10
            # ResBlock(16*c),
            ResBlock(16*c))
...
class IDM(nn.Module):
    def __init__(self, in_c, size):
        super(IDM, self).__init__()
        self.stn = SpatialTransformer(size)
        self.conv = nn.Conv3d(in_c, in_c, 3, 1, 1)
        self.idm = FIFM_Block(in_c)
        self.vec = VecInt(size=size)

    def forward(self, f_img, w_img):
        flow_f, M1 = self.idm(f_img)
        flow_m, M2 = self.idm(w_img)
        w1 = self.stn(M1, flow_m)
        w2 = self.stn(M2, flow_f)
        w = w_img + w1 + w2
        flow = self.stn(flow_f, flow_m) + self.stn(flow_m, flow_f)
        flow = self.vec(flow)
        return w, flow
...
class SDI(nn.Module):
    def __init__(self, channel):
        ...
        self.convs = nn.ModuleList(
            [nn.Conv3d(channel, channel, kernel_size=3, stride=1, padding=1) for _ in range(8)])
    ...
    def forward(self, xs, anchor):
        ...
        for i, x in enumerate(xs):
            ...
            if x.shape[-1] > target_size:
                x = F.adaptive_avg_pool3d(x, anchor.shape[2:])
            elif x.shape[-1] < target_size:
                x = F.interpolate(x, size=anchor.shape[2:], mode='trilinear', align_corners=True)
...
class Decoder(nn.Module):
    def __init__(self, size, in_channel, in_flow=True, use_da=False, use_de=False, use_swin=False, window_size=(128, 9, 11, 9)):
        ...
        self.resnets = ResNet(in_channel)
        self.pa = PAM_Module(in_channel)
 
        self.flow = nn.Conv3d(3 * in_channel, 3, 3, 1, 1)
        self.flows = nn.Conv3d(in_channel, 3, 3, 1, 1)
        
        self.nff = NFFBlock(3 * in_channel)
        if use_de:
            self.de = FIFM_Block(in_channel)
        if use_swin:
            self.swin = nn.Sequential(LGAMBlock3D(dim=in_channel, window_size=window_size),
                                    #   LGAMBlock3D(dim=2 * in_channel, window_size=window_size),
                                    #   LGAMBlock3D(dim=2 * in_channel, window_size=window_size),
                                      LGAMBlock3D(dim=in_channel, window_size=window_size)
                                      )
            
        self.convs = nn.Conv3d(3*in_channel, in_channel, 3, 1, 1)
        self.idm = IDM(in_channel, size=size)
        self.Corr = Correlation(max_disp=1, use_checkpoint=True)
        self.Conv = nn.Conv3d(27, in_channel, kernel_size=3, stride=1, padding='same')
      
    def forward(self, x, y, C, flow=None):
        corr = self.Conv(self.Corr(x, y))
        x = x+corr
        if self.in_flow:
            flow = self.upsample(flow*2)
            warp = self.stn(x, flow)
            if self.use_de:
                # flow_f, M1 = self.de(y)
                # flow_m, M2 = self.de(w)
                # w1 = self.stn(M1, flow_m)
                # w2 = self.stn(M2, flow_f)
                # x = x + w1 + w2
                w1, flow1 = self.idm(y, warp)
                warp, flow2 = self.idm(y, w1)
                flow_2 = self.stn(flow1, flow2) + flow2

        if self.use_swin:
            stack = self.convs(torch.cat([x, y, C], dim=1))
            stack = self.pa(stack)
            stack = self.resnets(stack)
            stack = self.swin(stack)
            flow = self.flows(stack)
        else:
            stack = self.nff(C, warp, y)
            stack = self.resnet(stack)
            flow = self.flow(stack)
            if self.use_de:
                # flow = flow + flow_f + flow_m
                flow = self.stn(flow_2, flow) + flow
        
        return flow

class LGANetPlusPlus(nn.Module):
    def __init__(self,
                 size=(160,192,160),
                 in_channel=1,
                 filters=8):
        super(LGANetPlusPlus, self).__init__()
        ...
        self.c4 = nn.Conv3d(8*filters,filters, 3, 1, 1)
        self.c3 = nn.Conv3d(4*filters,filters, 3, 1, 1)
        self.c2 = nn.Conv3d(2*filters,filters, 3, 1, 1)
        self.c1 = nn.Conv3d(filters, filters, 3, 1, 1)

        self.sdi_1 = SDI(filters)
        self.sdi_2 = SDI(filters)
        self.sdi_3 = SDI(filters)
        self.sdi_4 = SDI(filters)

        self.u4 = nn.Conv3d(filters, 8 * filters, 3, 1, 1)
        self.u3 = nn.Conv3d(filters, 4 * filters, 3, 1, 1)
        self.u2 = nn.Conv3d(filters, 2 * filters, 3, 1, 1)
        # self.deblock5 = Decoder(size=[s // 16 for s in self.size], in_channel=16*filters, in_flow=False, use_de=False, use_swin=True, window_size=(16*filters, 6, 12, 12))
        # self.deblock4 = Decoder(size=[s // 8 for s in self.size], in_channel=8*filters, in_flow=True, use_de=True)
        self.deblock4 = Decoder(size=[s // 8 for s in self.size], in_channel=8*filters, in_flow=False, use_de=False, use_swin=True, window_size=(8*filters, 6, 12, 12))
    ...
    def forward(self, moving, fixed):
        ...
        # encode stage
        M1, M2, M3, M4 = self.encoder(moving)  # 8 16 32 64 128
        ...
        C4 = self.sdi_4([f1, f2, f3, f4, m1, m2, m3, m4], f4)  # 1 8 h/8
        C3 = self.sdi_3([f1, f2, f3, f4, m1, m2, m3, m4], f3)  # 1 8 h/4
        C2 = self.sdi_2([f1, f2, f3, f4, m1, m2, m3, m4], f2)  # 1 8 h/2
        C1 = self.sdi_1([f1, f2, f3, f4, m1, m2, m3, m4], f1)  # 1 8 h w d
        
        C4 = self.u4(C4)
        flow4 = self.deblock4(M4, F4, C4)  # 1 3 18 22 18
        flow4 = self.integrate4(flow4)
        
        # flow4 = self.transformer[3](self.upsample_trilin(2*flow5), flow4)+flow4
        
        C3 = self.u3(C3)
        flow3 = self.deblock3(M3, F3, C3, flow4)  # 1 3 36 44 36
        flow3 = self.integrate3(flow3)
        flow3 = self.transformer[2](self.upsample_trilin(2*flow4), flow3)+flow3
        
        C2 = self.u2(C2)
        flow2 = self.deblock2(M2, F2, C2, flow3)  # 1 3 72 88 72
        flow2 = self.integrate2(flow2)
        flow2 = self.transformer[1](self.upsample_trilin(2*flow3), flow2)+flow2
        
        
        flow = self.deblock1(M1, F1, C1, flow2)  # 1 3 144 176 144
...
if __name__ == '__main__':
    inshape = (1, 1, 144, 176, 144)  # 6 12 12  12 24 24   24 48 48  48 96 96  96 192 192
    model = LGANetPlusPlus(size=(144, 176, 144))
